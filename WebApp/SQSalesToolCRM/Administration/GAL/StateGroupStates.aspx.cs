...
{
	public partial class StateGroupStates : SalesBasePage
	{

		public System.Data.SqlClient.SqlConnection SqlConnection = new System.Data.SqlClient.SqlConnection(CONFIG.ConnectionStrings["ApplicationServices"].ConnectionString); // ConfigurationManager.ConnectionStrings[SQS_Dialer.My.Settings.].ConnectionString);
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
             ...
             if (!IsPostBack)
            {
                ...
                StateGroupInfoDataSource.SelectParameters["bGALType"].DefaultValue = Master.GetGALType().ToString();
            }
            

			if (string.IsNullOrEmpty(Request.QueryString["key"])) {
				Session["StateGroupKey"] = null;
				Response.Redirect("StateGroupModify.aspx.aspx");
			} else {
				try {
					Session["StateGroupKey"] = Guid.Parse(Request.QueryString["key"].ToString());
				} catch (Exception ex) {
			...
			DataView dvSql = (DataView)StateGroupInfoDataSource.Select(DataSourceSelectArguments.Empty);

			foreach (DataRowView drvSql in dvSql) {
				AgentGroup.Text = drvSql["state_group_name"].ToString();
			}
		}
		...
		private void CampaignAddRemove(string MoveCommand)
		{
			...
			if (MoveCommand == "AddAll") {
				foreach (ListItem i in this.lstAvailableCampaigns.Items) {
					...
					InsertSQL = "INSERT INTO gal_StateGroupStates (stgrp_state_group_id, stgrp_state_id) VALUES('" + Session["StateGroupKey"].ToString() + "', '" + i.Value.ToString() + "')";
				...
				foreach (ListItem i in this.lstAddedCampaigns.Items) {
					string InsertSQL = null;
					InsertSQL = "DELETE FROM gal_StateGroupStates WHERE stgrp_id = '" + i.Value.ToString() + "'";
					SqlCommand.CommandText = InsertSQL;
					SqlCommand.CommandType = CommandType.Text;
