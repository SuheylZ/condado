...
{
    public class DialerConnectionMappings
    {
        ...
        public static List<string> GetConnectionID(string Userkey)
        {
            ...
            try
            {
                List<SignalR_GWB_Bindings> lstBindings = dbnew.SignalR_GWB_Bindings.Where(o => o.UserKey == Userkey).ToList();
                if (lstBindings != null)
                {
                    List<string> lstConnectionIDs = new List<string>();
                    lstBindings.ForEach(delegate(SignalR_GWB_Bindings o) { lstConnectionIDs.Add(o.ConnectionID); });
        ...
        public static List<string> GetconnectionsList(List<string> lstUsers)
        {
            ...
            try
            {
                List<SignalR_GWB_Bindings> lstBindings = new List<SignalR_GWB_Bindings>();
                lstBindings = dbnew.SignalR_GWB_Bindings.Where(o => lstUsers.Contains(o.UserKey)).ToList();
                if (lstBindings != null)
                {
                    List<string> lstConnectionIDs = new List<string>();
                    lstBindings.ForEach(delegate(SignalR_GWB_Bindings o) { lstConnectionIDs.Add(o.ConnectionID); });
        ...
        public SignalR_GWB_Bindings AddUser(string Userkey, string ConnectionID, string Token = "")
        {
            db = new SignalREntities();
            SignalR_GWB_Bindings binding = new SignalR_GWB_Bindings();
            ...
            {
                binding = db.SignalR_GWB_Bindings.Where(srb => srb.Token == Token).FirstOrDefault();
            }

            if (binding == null)
            {
                binding = new SignalR_GWB_Bindings();
                Token = Guid.NewGuid().ToString();
            }
            //binding = new SignalR_GWB_Bindings();
            binding.Token = Token;            
            ...
            db.SignalR_GWB_Bindings.Add(binding);
            db.SaveChanges();
        ...
        private void DeleteOldBindings()
        {
            ...
            List<SignalR_GWB_Bindings> lstBindings = new List<SignalR_GWB_Bindings>();
            lstBindings = (from srb in db.SignalR_GWB_Bindings
                           where (srb.DisconnectedTime < disconnectedTime 
                           && srb.DisconnectedTime != null)
                                 || srb.ConnectedTime < connectedTime
                           select srb).ToList();

            foreach (SignalR_GWB_Bindings binding in lstBindings)
            {
                db.SignalR_GWB_Bindings.Remove(binding);
            }
            db.SaveChanges();
        }
        ...
        {
            SignalR_GWB_Bindings srBinding = new SignalR_GWB_Bindings();
            srBinding = db.SignalR_GWB_Bindings.Where(u => u.Token == Token).FirstOrDefault();
            if (srBinding != null)
            {
        ...
        {
            SignalR_GWB_Bindings srBinding = new SignalR_GWB_Bindings();
            srBinding = db.SignalR_GWB_Bindings.Where(u => u.ConnectionID == ConnectionID).FirstOrDefault();
            if (srBinding != null)
            {
        ...
        {
            SignalR_GWB_Bindings objUser = db.SignalR_GWB_Bindings.Where(u => u.Token == Token).FirstOrDefault();
            if (objUser != null)
            {
                db.SignalR_GWB_Bindings.Remove(objUser);
                db.SaveChanges();
            }
        }
    }
