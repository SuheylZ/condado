...
AS
    SELECT TOP ( 5000 )
            ...
            dbo.Individuals.indv_key ,
            dbo.Individuals.indv_age ,
            dbo.Individuals.indv_birthday ,
            dbo.Individuals.indv_day_phone ,
            dbo.Individuals.indv_evening_phone ,
            dbo.Individuals.indv_cell_phone ,
            dbo.Individuals.indv_state_Id ,
            dbo.states.sta_full_name ,
            dbo.states.sta_abbreviation ,
            dbo.states.sta_key
    FROM    dbo.Accounts
    ...
    WHERE   ( dbo.statuses.sta_title IN ( N'New Real Time' ) )
            AND ( dbo.Accounts.act_assigned_usr IS NULL )
            AND ( dbo.Accounts.act_delete_flag = 0 )
            AND ( dbo.leads.lea_delete_flag = 0 )
            AND ( dbo.leads.lea_active_flag = 1 )
            AND ( dbo.campaigns.cmp_active_flag = 1 )
            AND ( dbo.campaigns.cmp_delete_flag = 0 )
            AND ( dbo.Individuals.indv_active_flag = 1 )
            AND ( dbo.Individuals.indv_delete_flag = 0 )
            AND ( ISNULL(dbo.leads.lea_first_contact_apt, GETDATE()) <= GETDATE() )
            AND ( dbo.Accounts.act_active_flag = 1 )
      ...
      Begin Tables = 
         ...
         Begin Table = "gal_campaigns"
            Begin Extent = 
               ...
               Right = 1149
            End
            Display', @level0type = N'SCHEMA', @level0name = N'dbo',
    @level1type = N'VIEW', @level1name = N'gal_leads_new';


GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPane2',
    @value = N'Flags = 280
            TopColumn = 7
         End
