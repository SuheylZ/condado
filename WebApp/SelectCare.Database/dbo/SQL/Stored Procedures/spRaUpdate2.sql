CREATE PROCEDURE [dbo].[spRaUpdate2] (@IsReal bit=1, @ruleId int=0)
                                    AS
                                    BEGIN
									...
									-- Start Dynamic Reassignment View Rules Queries Here     
                                      Print 'Rule #1 Reassign For XXXXXX start'
                                        insert into #REAL (rea_acct_key,rea_date,rea_priority,rea_usr_key,rea_usr_type)
                                        select distinct accounts.act_key, accounts.act_add_date, rea_priority = 1 , rea_usr_key = '88ad1695-7ddc-446c-855e-45fc0ddde980' , rea_usr_type = 1
                                      from accounts
                                      ...
                                      left JOIN carrier_issues ON carrier_issues.car_iss_act_key=act_key where ( CONVERT(Date,CONVERT(VARCHAR(10),dental_vision.den_effective_date,101)) = CONVERT(Date,CONVERT(VARCHAR(10),'1/1/2013',101)) ) 
                                        AND accounts.act_key not in (select rea_acct_key from #REAL) 
                                        Print 'Rule #1 Reassign For XXXXXX end'   Print 'Rule #2 Reassign For Self Generated Campaign to John Dobrotka start'
                                        insert into #REAL (rea_acct_key,rea_date,rea_priority,rea_usr_key,rea_usr_type)
                                        select distinct accounts.act_key, accounts.act_add_date, rea_priority = 2 , rea_usr_key = '7c496d0c-6ddc-4850-813e-f3d62d1f1ad1' , rea_usr_type = 2
                                      from accounts
                                      left JOIN dbo.Individuals on act_primary_individual_id = indv_key  
                                      left join individuals PrimaryIndividual on act_primary_individual_id =PrimaryIndividual.indv_key                                     
                                      left join individuals SecondaryIndividual on act_primary_individual_id = SecondaryIndividual.indv_key                                     
                                      ...
                                      left JOIN carrier_issues ON carrier_issues.car_iss_act_key=act_key where ( campaigns.cmp_id = '2' ) 
                                        AND accounts.act_key not in (select rea_acct_key from #REAL) 
                                        Print 'Rule #2 Reassign For Self Generated Campaign to John Dobrotka end'   Print 'Rule #3 Reassign For Busy start'
                                        insert into #REAL (rea_acct_key,rea_date,rea_priority,rea_usr_key,rea_usr_type)
                                        select distinct accounts.act_key, accounts.act_add_date, rea_priority = 3 , rea_usr_key = 'c07dd129-0c1c-45c3-9319-ee7e58166951' , rea_usr_type = 3
                                      from accounts
                                      left JOIN dbo.Individuals on act_primary_individual_id = indv_key  
                                      left join individuals PrimaryIndividual on act_primary_individual_id =PrimaryIndividual.indv_key                                     
                                      left join individuals SecondaryIndividual on act_primary_individual_id = SecondaryIndividual.indv_key                                     
                                      ...
                                      left JOIN carrier_issues ON carrier_issues.car_iss_act_key=act_key where ( Leads.lea_last_action = '29'  AND  (  NOT( Leads.lea_last_action_date Between ( DATEADD(MINUTE,-15,GETDate()) ) AND GETDATE() )  or Leads.lea_last_action_date is null )) 
                                        AND accounts.act_key not in (select rea_acct_key from #REAL) 
                                        Print 'Rule #3 Reassign For Busy end'   Print 'Rule #4 Reassign For Retention New start'
                                        insert into #REAL (rea_acct_key,rea_date,rea_priority,rea_usr_key,rea_usr_type)
                                        select distinct accounts.act_key, accounts.act_add_date, rea_priority = 4 , rea_usr_key = 'c07dd129-0c1c-45c3-9319-ee7e58166951' , rea_usr_type = 4
                                      from accounts
                                      left JOIN dbo.Individuals on act_primary_individual_id = indv_key  
                                      left join individuals PrimaryIndividual on act_primary_individual_id =PrimaryIndividual.indv_key                                     
                                      left join individuals SecondaryIndividual on act_primary_individual_id = SecondaryIndividual.indv_key                                     
                                      ...
                                      left JOIN carrier_issues ON carrier_issues.car_iss_act_key=act_key 
                                        AND accounts.act_key not in (select rea_acct_key from #REAL) 
                                        Print 'Rule #4 Reassign For Retention New end'   Print 'Rule #5 Reassign For DTE Census start'
                                        insert into #REAL (rea_acct_key,rea_date,rea_priority,rea_usr_key,rea_usr_type)
                                        select distinct accounts.act_key, accounts.act_add_date, rea_priority = 5 , rea_usr_key = '7c496d0c-6ddc-4850-813e-f3d62d1f1ad1', rea_usr_type = 5
                                      from accounts
                                      left JOIN dbo.Individuals on act_primary_individual_id = indv_key  
                                      left join individuals PrimaryIndividual on act_primary_individual_id =PrimaryIndividual.indv_key                                     
                                      left join individuals SecondaryIndividual on act_primary_individual_id = SecondaryIndividual.indv_key                                     
                                      ...
                                      left JOIN carrier_issues ON carrier_issues.car_iss_act_key=act_key where ( assigned_user.usr_key = 'c40df4be-4cc2-44c3-86e8-c49bbdeb4dff' ) 
                                        AND accounts.act_key not in (select rea_acct_key from #REAL) 
                                        Print 'Rule #5 Reassign For DTE Census end' 
