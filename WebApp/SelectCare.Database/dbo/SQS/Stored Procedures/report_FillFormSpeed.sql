...
if(@campaign is not null)
	...
	Select ROW_NUMBER() over(order by X.CaseSpecialist) as RowId, X.CaseSpecialist, isnull(X.FormCount,0) as FormCount, isnull(X.MaxMinutes,0) as MaxMinutes, isnull(X.MinMinutes,0) as MinMinutes , isnull(X.AvgMinutes,0) as AvgMinutes
	from (
