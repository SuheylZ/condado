...
sta_abbreviation,
sta_key)
select * from #temp gal_leads where act_key not in (select act_key from gal_leads_temp)
--AND gal_leads.act_add_date >= DATEADD(week, -1, GETDATE())

delete from gal_leads_temp where act_key not in (select act_key from #temp gal_leads /*WHERE gal_leads.act_add_date >= DATEADD(week, -1, GETDATE())   */)
...
INSERT INTO #groups*/
select campaign_group_id, gal_agents.agent_id, newest_available = max(act_add_date), oldest_available = MIN(act_add_date), available_leads = count(*) INTO #groups
...
SELECT
	pvt_usr_key=COALESCE(act_assigned_usr, act_transfer_user),
	pvt_count=Count(act_key),
	pvt_count_exceed =[dbo].[PVScheduleResult](COALESCE(act_assigned_usr, act_transfer_user)) INTO #gal_pv_counts
...
left join individuals (NOLOCK) pind on pind.indv_key = act_primary_individual_id
left join individuals (NOLOCK) sind on sind.indv_key = act_secondary_individual_id
where accounts.act_delete_flag != 1
    and COALESCE(act_assigned_usr, act_transfer_user) IS NOT null
	and (leads.lea_isduplicate <> 1 or leads.lea_isduplicate is null)	
	AND (pind.indv_key IS NOT NULL OR sind.indv_key IS NOT NULL)
GROUP BY COALESCE(act_assigned_usr, act_transfer_user)

UPDATE gal_pv_counts
SET pvt_count = #gal_pv_counts.pvt_count, pvt_count_exceed = #gal_pv_counts.pvt_count_exceed
FROM #gal_pv_counts
JOIN gal_pv_counts ON #gal_pv_counts.pvt_usr_key = gal_pv_counts.pvt_usr_key
WHERE #gal_pv_counts.pvt_count != gal_pv_counts.pvt_count OR gal_pv_counts.pvt_count_exceed != #gal_pv_counts.pvt_count_exceed --OR gal_pv_counts.pvt_count_exceed IS null

INSERT INTO gal_pv_counts (pvt_usr_key,pvt_count,pvt_count_exceed)
SELECT *
FROM #gal_pv_counts
WHERE pvt_usr_key not in (select pvt_usr_key FROM gal_pv_counts)

DELETE
FROM gal_pv_counts
WHERE pvt_usr_key in (SELECT pvt_usr_key
						FROM gal_pv_counts
						WHERE pvt_usr_key not in (select pvt_usr_key FROM #gal_pv_counts))
