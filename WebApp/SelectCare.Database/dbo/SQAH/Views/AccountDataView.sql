...
CREATE VIEW [dbo].[AccountDataView]
AS
SELECT TOP 100 PERCENT
					...
					PolicyType =
							CASE 
									...
									WHEN p.ahp_type = 4 THEN 'R.V.'
									WHEN p.ahp_type = 5 THEN 'Second Home'
									WHEN p.ahp_type = 6 THEN 'Fire Dwelling'
									WHEN p.ahp_type = 7 THEN 'Wind'
									WHEN p.ahp_type = 8 THEN 'Flood'
									WHEN p.ahp_type = 9 THEN 'Other'
									ELSE ''
							END,
					...
					WritingAgent =	
						CASE
							WHEN p.ahp_wag_usr_key IS NOT NULL THEN wa.usr_last_name + ' ' + wa.usr_first_name
							ELSE ''
						END,
					st.sta_title AS [Status],
					st1.sta_title AS SubStatus
					
FROM			dbo.Accounts a WITH (NOLOCK) LEFT JOIN dbo.leads l WITH (NOLOCK) ON a.act_key = l.lea_account_id
											...
											LEFT JOIN dbo.campaigns c WITH (NOLOCK) on c.cmp_id = l.lea_cmp_id
