...
-- =============================================
CREATE PROCEDURE [dbo].[spRptWklyAms]
...
BEGIN
	...
	BEGIN
		...
		--ahp_bound_date between ''4/8/13'' and getdate()
		...
		having min(ahp_bound_date) between ''4/8/13'' and getdate()
		...
		BEGIN
			EXEC msdb.dbo.sp_send_dbmail
				...
				@query = @sql,
				@subject = 'SelectCARE - AMS - SQAH',
				@query_attachment_filename = 'AMS_SQAH.csv',
	...
	BEGIN
		...
		BEGIN
			EXEC msdb.dbo.sp_send_dbmail
				...
				@recipients = @emaillist,
				@query = @sql,
				@subject = 'SelectCARE - AMS - CHOICEMARK',
				@query_attachment_filename = 'AMS_CM.csv',
				@attach_query_result_as_file = 1,
				@query_result_separator = @delimiter,
				@query_result_no_padding = 1,
				@exclude_query_output = 0
