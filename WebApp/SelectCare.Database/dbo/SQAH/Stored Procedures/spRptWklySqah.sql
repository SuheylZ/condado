...
-- =============================================
CREATE PROCEDURE [dbo].[spRptWklySqah]
...
BEGIN
	...
	BEGIN
		...
		where cmp_id not in (283, 254) and act_delete_flag = 0 and act_add_date between convert(varchar(2),MONTH(DATEADD(MONTH,-3,GETDATE()))) + ''/1/'' + convert(varchar(4),YEAR(DATEADD(MONTH,-3,GETDATE()))) and getdate() --convert(datetime, (convert(varchar(10), case when datepart(weekday, getdate()) >5 then DATEADD(DAY, +4, DATEADD(WEEK, DATEDIFF(WEEK, 0, getdate()), 0)) else DATEADD(DAY, -3, DATEADD(WEEK, DATEDIFF(WEEK, 0, getdate()), 0)) end, 101) + '' 23:59:59''))
	...
	'
		...
		BEGIN
			EXEC msdb.dbo.sp_send_dbmail
				...
				@query = @sql,
				@subject = 'SelectCARE - SQAH Metrics - All Leads',
				@query_attachment_filename = 'SQAHMetricsAllLeads.csv',
	...
	BEGIN
		...
		where cmp_id in (283, 254) and act_delete_flag = 0 and ach_added_date between convert(varchar(2),MONTH(DATEADD(MONTH,-3,GETDATE()))) + ''/1/'' + convert(varchar(4),YEAR(DATEADD(MONTH,-3,GETDATE()))) and getdate() --convert(datetime, (convert(varchar(10), case when datepart(weekday, getdate()) >5 then DATEADD(DAY, +4, DATEADD(WEEK, DATEDIFF(WEEK, 0, getdate()), 0)) else DATEADD(DAY, -3, DATEADD(WEEK, DATEDIFF(WEEK, 0, getdate()), 0)) end, 101) + '' 23:59:59''))
		...
		BEGIN
			EXEC msdb.dbo.sp_send_dbmail
				...
				@recipients = @emaillist,
				@query = @sql,
				@subject = 'SelectCARE - SQAH Metrics - INF',
				@query_attachment_filename = 'SQAHMetrics-INF.csv',
				@attach_query_result_as_file = 1,
				@query_result_separator = @delimiter,
				@query_result_no_padding = 1,
				@exclude_query_output = 0
	...
	BEGIN
		...
		[TA Name] = isnull(ta.usr_first_name,'''') + '' '' + isnull(ta.usr_last_name,''''),   
		[Create Date] = isnull(act_add_date,''''),
		[Bound Date - Auto] = (select isnull(MAX(autop.ahp_bound_date), '''') from  [selectcare].[dbo].[autohome_policies] autop join [selectcare].[dbo].[accounts] act on act.act_key = autop.ahp_act_id where  act.act_key = accounts.act_key and autop.ahp_type = 0 and autop.ahp_delete_flag = 0),
		[Bound Date - Home] = (select isnull(MAX(homep.ahp_bound_date), '''') from  [selectcare].[dbo].[autohome_policies] homep join [selectcare].[dbo].[accounts] act on act.act_key = homep.ahp_act_id where  act.act_key = accounts.act_key and homep.ahp_type = 0 and homep.ahp_delete_flag = 1),
		[Bound Premium - Auto] = (select isnull(SUM(case when autop.ahp_bound_date >= ''4/8/2013'' and autop.ahp_monthly_premium > 0 and autop.ahp_term > 0 then autop.ahp_monthly_premium / isnull(autop.ahp_term,12) else autop.ahp_monthly_premium end),0) from  [selectcare].[dbo].[autohome_policies] autop join [selectcare].[dbo].[accounts] act on act.act_key = autop.ahp_act_id where  act.act_key = accounts.act_key and autop.ahp_type = 0 and autop.ahp_delete_flag = 0),
		[Bound Premium - Renters] = (select isnull(SUM(case when rentp.ahp_bound_date >= ''4/8/2013'' and rentp.ahp_monthly_premium > 0 and rentp.ahp_term > 0 then rentp.ahp_monthly_premium / isnull(rentp.ahp_term,12) else rentp.ahp_monthly_premium end),0) from  [selectcare].[dbo].[autohome_policies] rentp join [selectcare].[dbo].[accounts] act on act.act_key = rentp.ahp_act_id where  act.act_key = accounts.act_key and rentp.ahp_type = 2 and rentp.ahp_delete_flag = 0),
		[Bound Premium - Umbrella] = (select isnull(SUM(case when umbrp.ahp_bound_date >= ''4/8/2013'' and umbrp.ahp_monthly_premium > 0 and umbrp.ahp_term > 0 then umbrp.ahp_monthly_premium / isnull(umbrp.ahp_term,12) else umbrp.ahp_monthly_premium end),0) from  [selectcare].[dbo].[autohome_policies] umbrp join [selectcare].[dbo].[accounts] act on act.act_key = umbrp.ahp_act_id where  act.act_key = accounts.act_key and umbrp.ahp_type = 3 and umbrp.ahp_delete_flag = 0),
		[Bound Premium - Home] = (select isnull(SUM(case when homep.ahp_bound_date >= ''4/8/2013'' and homep.ahp_monthly_premium > 0 and homep.ahp_term > 0 then homep.ahp_monthly_premium / isnull(homep.ahp_term,12) else homep.ahp_monthly_premium end),0) from  [selectcare].[dbo].[autohome_policies] homep join [selectcare].[dbo].[accounts] act on act.act_key = homep.ahp_act_id where  act.act_key = accounts.act_key and homep.ahp_type = 1 and homep.ahp_delete_flag = 0)
		...
		where act_delete_flag = 0 and act_key in (select ahp_act_id from [selectcare].[dbo].[autohome_policies] where ahp_bound_date between convert(varchar(2),MONTH(DATEADD(MONTH,-3,GETDATE()))) + ''/1/'' + convert(varchar(4),YEAR(DATEADD(MONTH,-3,GETDATE()))) and getdate())
		...
		BEGIN
			EXEC msdb.dbo.sp_send_dbmail
				...
				@recipients = @emaillist,
				@query = @sql,
				@subject = 'SelectCARE - SQAH Metrics - Bound',
				@query_attachment_filename = 'SQAHMetricsBound.csv',
				@attach_query_result_as_file = 1,
				@query_result_separator = @delimiter,
				@query_result_no_padding = 1,
				@exclude_query_output = 0
	...
	BEGIN
		set @sql = 
			...
			where act_delete_flag = 0 and ahq_quoted_date between dateadd(day,-90,getdate()) and getdate()
			...
			where act_delete_flag = 0 and ahq_id is null and ahp_bound_date between dateadd(day,-90,getdate()) and getdate()
		...
		BEGIN
			EXEC msdb.dbo.sp_send_dbmail
				...
				@recipients = @emaillist,
				@query = @sql,
				@subject = 'SelectCARE - SQAH Metrics - Quote',
				@query_attachment_filename = 'SQAHMetricsQuote.csv',
				@attach_query_result_as_file = 1,
				@query_result_separator = @delimiter,
				@query_result_no_padding = 1,
				@exclude_query_output = 0
